...
- 3.目前使用的方法，通过组件定义openDialog方法并通过defineExpose抛出，每次打开时在openDialog中调用表单的clearVilate清楚校验方法
...
- 1.v-model 的形式
``` html
<addMetricDialog v-if="dialogForm.visible" v-model="dialogForm" @createMetric="createMetric" /> 
```
